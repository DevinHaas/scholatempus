# Agent Guide for scholatempus Backend
...
- **Shared Exports:** Use `./shared` and `./shared/*` exports pattern defined in `package.json`

## Encore.ts Conventions

- **API Endpoint Structure:** Use `api({ expose, path, method, auth }, async handler)` pattern
- **Authentication:** Use `getAuthData()` from `~encore/auth` for auth-required endpoints
- **Error Handling:** Use `APIError` with appropriate error codes (e.g., `APIError.unauthenticated()`, `APIError.internal()`)
- **Logging:** Use `encore.dev/log` for structured logging with context objects
- **Service Structure:** Services defined in `encore.service.ts` files

## Database Patterns (Drizzle ORM)

- **Schema Location:** `db/schema.ts`
- **Database Connection:** `db/index.ts` exports drizzle instance using PostgreSQL connection pool
- **Migration Management:** Use Drizzle Kit for migrations:
  - Generate: `drizzle-kit generate`
  - Migrate: `drizzle-kit migrate`
- **Schema Conventions:**
  - Use enums from `~/shared` for enum types (e.g., `GradeLevel`, `WorkTimeCategory`)
  - Define relations using `relations()` helper from `drizzle-orm`
  - Use proper foreign key constraints with cascade deletes where appropriate
  - Use `InferSelectModel` from Drizzle for database types when needed

## Authentication (Clerk)

- **Auth Handler:** `auth/auth.ts` defines `myAuthHandler`
- **Gateway:** `mygw` Gateway instance with auth handler
- **Auth Data Structure:** `{ userID, imageUrl, emailAddress }`
- **Token Verification:** Uses Clerk's `verifyToken` with authorized parties from `auth/config.ts`
- **Secret Management:** `CLERK_SECRET_KEY` from environment variables (use `@dotenvx/dotenvx/config`)

## API Endpoint Conventions

- **Request/Response Types:** Define in `shared/api-types.ts`
- **Validation:** Use Zod schemas from `shared/schemas.ts`
- **Type Imports:** Use `~/shared` alias for shared types
- **Error Responses:** Consistent `APIError` usage with appropriate codes
- **Transaction Handling:** Use `db.transaction()` for multi-step database operations

## Code Organization

- **Services:** Organized by feature (e.g., `app/profile/`, `app/workentries/`)
- **Shared Code:** `shared/` directory for types, schemas, enums
- **Database:** `db/` directory for schema and connection
- **Auth:** `auth/` directory for authentication logic

## Type Safety

- Use TypeScript interfaces for API contracts
- Derive types from Zod schemas where possible
- Import enums from `shared/enums.ts`
- Use `InferSelectModel` from Drizzle for database types

## Error Handling Patterns

- Always check auth data before proceeding: `const authData = getAuthData(); if (!authData) { throw APIError.unauthenticated(...); }`
- Use try-catch blocks for error handling
- Log errors with context using structured logging
- Re-throw `APIError` instances, wrap others in `APIError.internal()`

## Logging Best Practices

- Use structured logging with context objects: `log.info("message", { userId, ...context })`
- Include `userId` in log context when available
- Use appropriate log levels (info, error, warn)
- Log important operations (creates, updates, deletes)

## Environment & Configuration

- Use `@dotenvx/dotenvx/config` for environment variables
- **Database Connection:** `DATABASE_URL` environment variable
- **Clerk Secret:** `CLERK_SECRET_KEY` environment variable
- **CORS Configuration:** Defined in `encore.app` file under `global_cors` key
